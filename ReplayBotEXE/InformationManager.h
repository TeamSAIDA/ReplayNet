...
namespace MyBot
{
	...
	class InformationManager 
	{
		...
		/// 맵 이름 (맵 파일이름이 조금 달라지더라도, 맵 이름으로 전략을 선택)
		std::string													mapName;
		...
		void                    addUnitInfo(BWAPI::Unit unit);
		// enemy 전용 ( Hide, Show )

		void					updateBaseLocationInfo();
		void					updateChokePointAndExpansionLocation();
		void					updateOccupiedRegions(BWAPI::Player player);
...
//		void					onUnitHide(BWAPI::Unit unit)        { updateUnitHide(unit, true); }
		...
		/// 현재 맵의 이름을 간략화 및 소문자로 변환하여 리턴합니다 
		/// 예시 : (4)Python.scx -> python 을 리턴
		/// 맵 파일이름이 조금 달라지더라도, 맵 이름으로 전략을 선택할 수 있도록 하기 위함
